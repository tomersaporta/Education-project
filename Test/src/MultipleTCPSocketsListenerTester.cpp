...
void MultipleTCPSocketsListenerTester::run() {
	...
	for (int j = 0; j < TEST_SIZE; j++) {
		...
		if (aClient != NULL) {
			...
			int rec = aClient->recv((char*) &len, 4); // message size
			if(rec<0){
			...
			aClient->send((char*) &len, sizeof(len));
			aClient->send(message.data(), message.length());
		}
...
void MultipleTCPSocketsListenerTester::createClientAndSendRecvMsg(int port){
	...
	for (int i=0;i<1;i++){
		...
		readtCSocket->recv((char*)&len,4);
		len = ntohl(len);
		int rc = readtCSocket->recv(buffer, len);
		cout<<"receive message:"<<buffer<<endl;
